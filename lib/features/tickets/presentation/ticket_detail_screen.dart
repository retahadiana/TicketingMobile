...
class _TicketDetailScreenState extends ConsumerState<TicketDetailScreen> {
  ...
  bool _isSupport(Profile? profile) {
    ...
    return profile.role == UserRole.helpdesk || profile.role == UserRole.admin;
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            if (ticket.imageUrl != null) ...<Widget>[
              const SizedBox(height: 12),
              Text('Lampiran: ${ticket.imageUrl}'),
            ],
            const SizedBox(height: 20),
            Text('Tracking Status', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            ...TicketStatus.values.map((status) {
              final isDone = TicketStatus.values.indexOf(status) <= currentStatusIndex;
              return ListTile(
